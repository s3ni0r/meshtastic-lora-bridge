...
| `0x05` TRACK | `sub u8`: `0x00` BEGIN (`count u16, crc32 u32`) · `0x01` CHUNK (`offRec u16, n u8, n×10 B records`) · `0x02` COMMIT · `0x03` ABORT | uploads the replay slot (`/prefs/simtrack.bin`, ≤1600 records — LittleFS is 28 KB total). Record: `lat i32, lon i32, speed u8 km/h, dt u8` (0.1 s from previous point). COMMIT re-CRCs the file — a half/corrupt upload can never play. **BLE-direct/USB only by convention** (bulk over LoRa would eat the duty budget); playback then needs no link. The tag interpolates between records at its configured fix cadence. Bench-validated 2026-07-26: 60-record square uploaded in 3 chunks (all `0x85 status 0`), replay walked the exact course at 15 km/h with corner turns, loop + STOP clean. iOS: Simulator card → Import GPX / From session → upload with progress → Play track |
