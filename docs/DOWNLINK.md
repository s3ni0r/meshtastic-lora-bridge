...
| `0x05` TRACK | `sub u8`: `0x00` BEGIN (`count u16, crc32 u32`) · `0x01` CHUNK (`offRec u16, n u8, n×10 B records`) · `0x02` COMMIT · `0x03` ABORT | uploads the replay slot (`/prefs/simtrack.bin`, ≤1600 records — LittleFS is 28 KB total). Record: `lat i32, lon i32, speed u8 km/h, dt u8` (0.1 s from previous point). **Reply is a CORRELATED ACK, not the generic settings frame: `[0x85, status, sub, offLo, offHi]`** — the client must match (sub, off) exactly (a stale/foreign 0x85 never credits a frame). Chunks are duplicate-tolerant (a re-send of the last banked chunk ACKs without rewriting). **COMMIT verifies the stored CRC, then writes a durable marker file (`simtrack.ok`) — only a committed slot plays, across reboots; repeated COMMIT is idempotent** (a lost COMMIT reply + retry still reports success). **BLE-direct/USB only by convention** (bulk over LoRa would eat the duty budget); playback then needs no link. The tag interpolates between records at its configured fix cadence. Bench-validated 2026-07-26 (see `tools/bench/`): upload+replay PASS, duplicate-chunk ACK PASS, partial/uncommitted slot REFUSES to play. iOS: Simulator card → Import GPX / From session → ACK-verified upload with progress → Play track |
