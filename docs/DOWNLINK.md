...
| `0x05` TRACK | `sub u8`: `0x00` BEGIN (`count u16, crc32 u32, nonce u8` — nonce ≠ 0, chosen per upload) · `0x01` CHUNK (`offRec u16, n u8, n×10 B records`) · `0x02` COMMIT · `0x03` ABORT | uploads the replay slot (≤1600 records; record: `lat i32, lon i32, speed u8 km/h, dt u8` 0.1 s from previous point). **Phone/USB-direct ONLY — mesh-relayed TRACK ops are rejected with a NAK** (destructive op, R3 f7). **Staging is transactional**: uploads build `simtrack.tmp`; the committed live slot (`simtrack.bin` + `simtrack.ok` marker) stays playable until COMMIT verifies the replacement's CRC and swaps it in — a stray BEGIN/ABORT or a mid-upload reboot can never destroy a valid track. **Reply: `[0x85, status, sub, offLo, offHi, nonce]`** — the client must match sender node + nonce + sub + offset exactly (a stale ACK from a previous upload or another tag can never credit a frame, R3 f3). Duplicate handling is exact: only a re-send of the previous chunk (same offset AND length) ACKs without rewriting. Repeated COMMIT is idempotent. The tag interpolates between records at its configured fix cadence; playback needs no link. Bench (tools/bench/, hard-asserted with exit codes): correlation per frame, dup-chunk, COMMIT retry, staging survival across stray BEGIN/ABORT, and reboot durability — ALL PASS 2026-07-26. iOS: nonce-verified upload bound to one tag + one connection generation, cancelled on screen/target change |
...
MeshTracker's config client does exactly that and refuses mismatched peripherals.
